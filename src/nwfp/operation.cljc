(ns nwfp.operation
  "NwfpOperationActor -- one NWFP-gathering operation = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor (NwfpAdvisor)
  is sealed into a single node (:advise); its proposal is ALWAYS routed
  through the NWFP Gathering Coordination Governor (:govern) and the rollout
  phase gate (:decide) before anything commits to the SSoT.

  One graph run = one gathering operation (assess -> advise -> govern ->
  ...
  MANY ops (harvest-record logging / trip scheduling / concern flags), each
  independent.

  Human-in-the-loop = real approval workflow:
  `interrupt-before #{:request-approval}` pauses the actor and hands the
  decision to a human gathering-operations manager. The approver resumes
  with `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [nwfp.advisor :as advisor]
            [nwfp.governor :as governor]
            [nwfp.phase :as phase]
            [nwfp.store :as store]))
...
(defn build
  "Compiles a NwfpOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `nwfp.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; NwfpAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; NWFP Gathering Coordination Governor -- independent censor.
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        ...
                        :reason (or reason
                                    (cond (:high-stakes? verdict) :requires-approval
